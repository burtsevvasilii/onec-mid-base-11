...
Процедура ВыполнитьКоманду(Знач Форма, Знач ПараметрыВызова, Знач Источник = Неопределено, Результат = Неопределено) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ВыполнитьМетодКонфигурации(Обработчик, ПараметрыЭкспортнойПроцедуры);
...
Функция ПодключенныеОбъекты(ОписаниеИсточника, ПодключенныеОбъекты = Неопределено, НастройкиПрограммногоИнтерфейса = Неопределено) Экспорт
	...
	Если ТипЗнч(ОписаниеИсточника) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных") Тогда
		...
		Источник.ТипСсылкиДанных = ОбщегоНазначенияКлиентСервер.ЗначениеРеквизитаОбъекта(ОписаниеИсточника, "ЗначениеПустойСсылки");
	Иначе
	...
	Для Каждого ПолноеИмя Из ПолныеИменаПодключенныхОбъектов Цикл
		...
		Если ПодключенныйОбъект = Неопределено Тогда
			...
			ПодключенныйОбъект.Метаданные = ОбщегоНазначенияКлиентСервер.ОбъектМетаданныхПоПолномуИмени(ПолноеИмя);
		Иначе
...
Функция НастройкиПодключаемогоОбъекта(ПолноеИмя, НастройкиПрограммногоИнтерфейса = Неопределено) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			...
			ОбщегоНазначенияКлиентСервер.ОбъектМетаданныхПоПолномуИмени(ПолноеИмя),
			ПолноеИмя,
			ТекстОшибки);
		Возврат Неопределено;
	КонецПопытки;
...
Функция ЗарегистрироватьИсточник(ОбъектМетаданных, Источники, ПодключенныеОбъекты, НастройкиПрограммногоИнтерфейса) Экспорт
	...
	Менеджер  = ОбщегоНазначенияКлиентСервер.МенеджерОбъектаПоПолномуИмени(ПолноеИмя);
	...
	Источник.СсылкаМетаданных    = ОбщегоНазначенияКлиентСервер.ИдентификаторОбъектаМетаданных(ПолноеИмя);
...
Функция КэшФормы(ИмяФормы, ИсточникиЧерезЗапятую, ЭтоФормаОбъекта) Экспорт
	...
	Иначе
		...
		Для Каждого ПолноеИмя Из ПолныеИменаИсточников Цикл
			ОбъектМетаданных = ОбщегоНазначенияКлиентСервер.ОбъектМетаданныхПоПолномуИмени(ПолноеИмя);
	...
	Для Номер = 1 По Количество Цикл
		...
		Если ПустаяСтрока(Команда.Идентификатор) Тогда
			Команда.Идентификатор = "Авто_" + ОбщегоНазначенияКлиентСервер.КонтрольнаяСуммаСтрокой(Команда.Менеджер + "/" + Команда.ИмяФормы + "/" + Команда.Обработчик);
		КонецЕсли;
	КонецЦикла;
...
Функция КлючПараметровРазмещения(Знач ПараметрыРазмещения)
	...
	ПараметрыРазмещения = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ПараметрыРазмещения);
	...
	Возврат ОбщегоНазначенияКлиентСервер.КонтрольнаяСуммаСтрокой(ОбщегоНазначенияКлиентСервер.ЗначениеВСтрокуXML(ПараметрыРазмещения));
...
Функция ОперативноеОбновлениеОбщихДанных(Отбор)
	...
	Для Каждого ОбъектМетаданныхПоставщика Из Состав Цикл
		Если Не ОбщегоНазначенияКлиентСервер.ДоступноИспользованиеРазделенныхДанных() И ОбъектМетаданныхПоставщика.РасширениеКонфигурации() <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ИдентификаторОбъектаМетаданных = ОбщегоНазначенияКлиентСервер.ИдентификаторОбъектаМетаданных(ОбъектМетаданныхПоставщика, Ложь);
		...
		Для Каждого ОбъектМетаданных Из Настройки.Размещение Цикл
			ИдентификаторОбъектаМетаданных = ОбщегоНазначенияКлиентСервер.ИдентификаторОбъектаМетаданных(ОбъектМетаданных);
	...
	Если Не ОбщегоНазначенияКлиентСервер.ДанныеСовпадают(СтароеЗначение, НовоеЗначение) Тогда
...
Функция ОписаниеКоманды(ИмяКомандыВФорме, АдресНастроек) Экспорт
	...
	Если Команда = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ИмяКомандыВФорме);
	КонецЕсли;
	ОписаниеКоманды = ОбщегоНазначенияКлиентСервер.СтрокаТаблицыЗначенийВСтруктуру(Команда);
